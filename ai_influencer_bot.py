...
class AIContentBot:
    ...
    def run_hourly(self):
        ...
        global _last_run_time, _total_runs
...
def setup_telegram_handlers():
    ...
    def status(update, context):
        ...
        update.message.reply_text(
            ...
            f"🖥️ KeepAlive: http://{KEEPALIVE_HOST}:{KEEPALIVE_PORT}",
            parse_mode='Markdown'
        )
    ...
    def health(update, context):
        ...
        update.message.reply_text(
            ...
            f"Last Run: {stats.get('last_run', 'N/A')}",
            parse_mode='Markdown'
        )
    ...
    def stats_command(update, context):
        ...
        update.message.reply_text(
            ...
            f"Total Posts: {stats.get('total_posts_generated', 0)}\n"
            f"Trends Tracked: {stats.get('total_trends_scraped', 0)}\n"
            f"Avg Engagement: {stats.get('average_engagement_score', 0)}\n"
            f"Total Runs: {stats.get('total_runs', 0)}",
            parse_mode='Markdown'
        )
...
def main():
    ...
    print()
    print(f"🌐 KeepAlive server will run on port {KEEPALIVE_PORT}")
    print("   Use UptimeRobot to ping: http://YOUR_RENDER_URL/health")
    print()
